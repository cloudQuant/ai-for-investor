...
本特性在 ai-for-investor 前端现有的局部国际化（i18n）基础设施之上，扩展为一套完善的多语言支持方案。当前前端已集成 vue-i18n（Composition API 模式），但仅注册了简体中文（zh-CN）与英文（en-US）两套语言包，且语言切换组件 `LanguageSwitcher.vue` 尚未挂载到任何布局中。
