...
本文档定义了 ai-for-investor 量化交易平台**迭代 175**的需求。迭代 175 在迭代 174「结构精简与工程债接续」基础上推进，聚焦于把已经搭好的骨架进一步收紧到「可观测、可验证、可阻塞回归」的状态。
...
- **175 不做**：任何新业务功能 PRD；不新增框架/语言/构建工具；不动数据库 schema（除迁移守护脚本本身）；不在 `ai_for_investor` 内新增 broker 适配；不重做 UI 信息架构。
