...
from app.services.market_data.query_cursor import (
    _DEFAULT_CURSOR_TTL,
    MarketDataCursorBinding,
    MarketDataQueryServiceError,
    _assert_cursor_access_grant_matches,
    _binding_for_access,
    _coerce_cursor_signing_key,
    _Cursor,
    _decode_cursor,
    _paginate_observations,
    _policy_descriptor_hash,
    _require_text,
)
...
class _Store(Protocol):
    ...
    async def persist_provider_result(
        ...
        *,
        ...
        """Append a source receipt, which may remain hidden pending promotion."""
        ...
        for window in fetch_windows:
            ...
            try:
                ...
                for route in routes:
                    ...
                    if isinstance(persisted, DeferredProviderFetch):
                        # A staged receipt has no trusted visibility instant. It
                        # cannot be represented as a query fetch or advance any
                        # local read boundary until the guarded promotion path
                        # returns a normal persisted receipt.
                        raise MarketDataQueryServiceError("PROVIDER_RECEIPT_NOT_VISIBLE")
