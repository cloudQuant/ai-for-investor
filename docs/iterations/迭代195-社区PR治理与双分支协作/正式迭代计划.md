...
> 适用仓库：`cloudQuant/ai-for-investor`
...
9. `CONTRIBUTING.md` 中的 `YOUR_USERNAME/ai_for_investor.git` 是 fork 克隆占位符，不是错误仓库名；应补充替换说明和 `upstream` 配置，而不是改成所有贡献者都克隆上游。
...
```bash
gh repo view cloudQuant/ai-for-investor --json defaultBranchRef,visibility
gh api repos/cloudQuant/ai-for-investor/rulesets --paginate
gh api repos/cloudQuant/ai-for-investor/branches/master/protection || true
gh api repos/cloudQuant/ai-for-investor/branches/dev/protection || true
gh api repos/cloudQuant/ai-for-investor/codeowners/errors
gh label list --repo cloudQuant/ai-for-investor --limit 100
git ls-remote --heads https://github.com/cloudQuant/ai-for-investor.git master dev
git ls-remote --heads https://gitee.com/yunjinqi/ai-for-investor.git master dev
```
...
1. `CONTRIBUTING.md` 明确 fork 后将 `YOUR_USERNAME` 替换成自己的用户名，添加 `git remote add upstream https://github.com/cloudQuant/ai-for-investor.git`，并写明 feature/fix 基于 `dev`、release/hotfix 的专用起点与目标。
...
/Users/yunjinqi/opt/anaconda3/bin/conda run -n base python \
  scripts/ci/check_remote_sync.py --source https://github.com/cloudQuant/ai-for-investor.git \
  --mirror https://gitee.com/yunjinqi/ai-for-investor.git --branches master dev \
  "${REMOTE_SYNC_EXCEPTION_ARGS[@]}"
...
默认演练使用本地 fixture 和不可 push 的 `workflow_dispatch` dry-run，分别模拟 tag 不在 `master` HEAD（必须在 Docker login 前失败）与 tag 正确（只生成 metadata/build artifact，不登录、不发布）；**不得**在 `cloudQuant/ai-for-investor` 创建或推送演练 tag。只有 D6 记录了独立 staging 仓库、受保护 staging environment 和明确授权时，才可在该仓库用符合生产格式的测试 tag 做端到端演练。生产正向证据来自下一次真实、已批准的 release，不为验收伪造版本或 digest。
